...
$lista = [];
$sql = $conn->query("SELECT * FROM produtos_refrigerados");
if($sql->rowCount() > 0) {
...
<h1>Controle dos Produtos Refrigerados</h1>
...
<thead>
    ...
    <?php foreach($lista as $produtos):?>
        ...
        <td><?=date('d/m/Y',strtotime($produtos['data_ent']));?></td>
        <td><?=date('d/m/Y',strtotime($produtos['data_s']));?></td>
        <td><?=$produtos['armaz_temp_max'];?></td>
...
<td>
            ...
            <a href="cadastrar_prodref.php?id=<//?=$produtos['id'];?>">
    ...
    -->
        <td>
            <a href="editar_prodref.php?id=<?=$produtos['id'];?>">
            <img class="del" src="../IMG/pen.png"></a>
        </td>    

        <td>
            <a class='trash3' href="excluir_prodref.php?id=<?=$produtos['id'];?> ">
